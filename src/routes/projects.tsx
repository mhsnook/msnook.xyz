...
export const Route = createFileRoute('/projects')({
	loader: async () => (await fetchProjects()) ?? [],
	staleTime: 60_000,
	head: () => ({
		meta: [
			{ title: 'Projects – em snook' },
			{ name: 'description', content: "Things I've been building" },
		],
	}),
		...
		<>
			...
			<main className="container py-10">
				<div className="max-w-4xl mx-auto mb-4 flex justify-end">
					<IffLoggedIn>
						{/* /projects/manage still lives on the Next deploy */}
						<a
								...
								<div className="flex items-center justify-between pt-2">
									...
									<div className="flex items-center gap-3 ml-3 shrink-0">
										{project.url?.startsWith('http') ? (
											...
											// Dynamic URL from Supabase — not statically typeable against
											// the route tree, so use a plain <a>. Still client-side-routed
											// if the path matches a Start route.
											<a
												href={project.url}
												className="text-sm font-medium text-cyan-bright hover:underline whitespace-nowrap"
											>

...
export default function CycleTimeline({ cycle, dayOffset, theme }: Props) {
	...
	const dayInCurrentPhase = Math.round(
		(cycle.cycleStart.getTime() + dayOffset * 86_400_000 - currentPhase.start.getTime()) /
			86_400_000,
	)
	const fractionInPhase = currentPhase.days > 1 ? dayInCurrentPhase / currentPhase.days : 0

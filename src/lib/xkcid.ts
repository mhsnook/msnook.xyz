...
function parseNumberFromBaseX(variant: XkcidVariantOptions, baseXStr: BaseXString): number {
	const { alphabet } = settings[variant]
	const base = alphabet.length
...
function padString(variant: XkcidVariantOptions, inputString: BaseXString): BaseXString {
	const { alphabet, timeLength } = settings[variant]
	if (timeLength === inputString.length) return inputString
	return `${Array.from({ length: timeLength - inputString.length }, () => alphabet[0]).join(
		'',
	)}${inputString}`
}

function trimString(variant: XkcidVariantOptions, inputString: BaseXString): BaseXString {
	const { timeDropPrecision } = settings[variant]
	const trimmedTimeString = inputString.substring(0, inputString.length - timeDropPrecision)
...
function xkcid(variant: XkcidVariantOptions = 'lowercase') {
	...
	const result = {
		...
		randomnessSizeInMillions: Math.round(Math.pow(base, randomnessLength) / 100_000) / 10,
		testBackAndForth: (value: Transformable, goTimes: number = 2) => {
			...
			while (goTimes > 0) {
				...
				if (typeof recentest === 'number') result.push(numberToBaseX(variant, recentest))

...
const FOCUS_CAUSE_IDS = [101, 102, 103, 104, 106, 107, 201, 202, 203, 204, 208, 301, 302, 303]
...
export default function MortalityExplorer({ data }: { data: MortalityData }) {
	...
	const [collapsedSuperRegions, setCollapsedSuperRegions] = useState<Set<number>>(new Set())
	...
	function getCellValue(causeId: number, regionId: number): number | null {
		...
		return ((rate.age_standardized_rate - baseline.rate) * region.population) / 100000
	}
	...
	const sortedSuperRegions = useMemo(() => {
		if (sort.key === 'region') {
			return [...superRegions].sort((a, b) =>
				sort.direction === 'asc' ? a.sort_order - b.sort_order : b.sort_order - a.sort_order,
			)
		}
		return superRegions
	}, [superRegions, sort])
		...
		for (const cause of visibleCauses) {
			...
			for (const region of regions) {
				...
				if (rate?.age_standardized_rate && region.population) {
					const excess = ((rate.age_standardized_rate - baseline.rate) * region.population) / 100000
	...
	return (
		<div>
			...
			<div className="mb-4 p-3 bg-gray-50 rounded border text-sm">
				...
				<span className="text-red-700 font-bold">{(grandTotalExcess / 1_000_000).toFixed(1)}M</span>
				...
				<table className="w-full text-xs border-collapse">
					<thead>
						...
						<tr className="bg-gray-50">
							<th className="sticky left-0 z-10 bg-gray-50 border-b border-r px-2 py-1" rowSpan={2}>
								<button
									onClick={() => handleSort('region')}
									className="font-bold hover:text-cyan-bright"
								>
									Region
									{sort.key === 'region' ? (sort.direction === 'asc' ? ' ↑' : ' ↓') : ''}
								</button>
							</th>
							{level1Causes.map((l1) => {
								...
								return (
									...
									>
										{l1.id === 100 ? 'CMNN' : l1.id === 200 ? 'NCD' : 'Injuries'}
									</th>
								)
							})}
							<th className="border-b border-r px-2 py-1 font-bold text-gray-500" rowSpan={2}>
								Total
							</th>
						</tr>
						{/* Level 2 cause names */}
							...
							{visibleCauses.map((cause) => (
								...
								>
									...
									>
										...
										{sort.key === cause.id ? (sort.direction === 'asc' ? ' ↑' : ' ↓') : ''}
									</button>
								</th>
							))}
						</tr>
						...
						<tr className="bg-emerald-50">
							...
							<td className="border-b border-r px-1 py-1 text-center text-emerald-700">—</td>
						</tr>
					</thead>
					<tbody>
								...
								<>
											...
											return (
												<td
													...
													className={`border-b border-r px-1 py-1 text-center font-medium ${
														avg !== null
															? rateColor(
																	viewMode === 'rates' ? avg + (baseline?.rate ?? 0) : avg,
																	baseline?.rate ?? 0,
																	colMax,
																	viewMode,
																)
									...
									{!isCollapsed &&
										srRegions.map((region) => (
											<tr key={`r-${region.id}`} className="hover:bg-blue-50/50">
												...
												{visibleCauses.map((cause) => {
													...
													const baseline = baselineMap.get(cause.id)
													const rawRate =
														rateMap.get(`${cause.id}-${region.id}`)?.age_standardized_rate ?? null
													const colMax = columnMaxes.get(cause.id) || 1
					...
					<tfoot>
						...
						<tr className="bg-gray-50 font-bold">
							...
							{visibleCauses.map((cause) => (
								<td key={cause.id} className="border-t-2 border-r px-1 py-1 text-center">
									{formatNumber(totalExcessByCause.get(cause.id) ?? null, 'excess_deaths')}
								</td>
							))}
			...
			<div className="mt-4 grid gap-4 md:grid-cols-2">
				<div className="text-xs text-gray-500">
					<p className="font-bold text-gray-700 mb-1">Color scale (per column)</p>
				...
				<div className="text-xs text-gray-500">
					...
					<p>
						Each cause uses the lowest-rate large GBD region as its baseline. Excess = actual rate
						minus baseline rate. &ldquo;Excess deaths&rdquo; multiplies the excess rate by the
						region&rsquo;s population.
					</p>
				</div>
			</div>
			...
			{/* Top causes table */}
			<div className="mt-6">
				<h2 className="h4 mb-2">Largest causes of excess mortality (estimated)</h2>
						...
						.map(([causeId, total], i) => {
							...
							return (
								<div key={causeId} className="flex items-center gap-2 text-sm">
									...
									<span className="text-red-700 font-bold">{(total / 1_000_000).toFixed(1)}M</span>
								</div>
							)
						})}
				</div>

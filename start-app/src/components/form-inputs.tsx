...
// The form fields are loose on purpose — these inputs get reused across
// `create` and `edit` shapes of Tables<'posts'>. We type against a
// record-like interface and let the caller supply the shape.
// eslint-disable-next-line @typescript-eslint/no-explicit-any
type Register = UseFormRegister<any>
// eslint-disable-next-line @typescript-eslint/no-explicit-any
type SetValue = UseFormSetValue<any>
// eslint-disable-next-line @typescript-eslint/no-explicit-any
type GetValues = UseFormGetValues<any>

// react-hook-form's errors[key] can be FieldError OR a Merge<FieldError,
// FieldErrorsImpl> for nested fields. The inputs below only care about
// truthiness + optional message, so widen to a structural shape that
// both satisfy.
...
export function InputSlug({ register, error }: { register: Register; error?: FormError }) {
	return (
		<div>
...
export function InputContent({
	...
	const insertImageAtCursor = (url: string, altText: string) => {
		...
		const newCursorPos =
			cursorPos + (needNewlineBefore ? 1 : 0) + markdownImage.length + (needNewlineAfter ? 1 : 0)
		setTimeout(() => {
			if (textarea) {
	...
	return (
		<div>
			...
			<div className="flex items-center gap-2 mt-1">
				...
				{uploadImage.isPending && <span className="text-sm text-gray-500">Uploading...</span>}
			</div>
			{uploadImage.error && (
				<ErrorList summary="Error uploading image" error={uploadImage.error.message} />
			)}
		</div>
	)
}

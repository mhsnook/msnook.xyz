...
import { getORP, tokenize } from '@/lib/reader-tokenizer'
...
export const Route = createFileRoute('/reader')({
	head: () => ({
		meta: [
			...
			{
				name: 'description',
				content: 'Rapid Serial Visual Presentation reader — PDF and plain text',
			},
		],
	}),
	component: RSVPReader,
	ssr: false,
})
...
const FONT_CONFIG: Record<FontChoice, { family: string; label: string; orpWidth: number }> = {
	mono: {
...
function savePosition(hash: string, name: string, idx: number, wpm: number, wordCount: number) {
...
function RSVPReader() {
	...
	useEffect(() => {
		...
		const id = setInterval(() => {
			savePosition(textHash, textName, idxRef.current, wpmRef.current, words.length)
		}, 3000)
	...
	const getSentenceText = useCallback(
		(sentIdx: number) => {
			...
			const end = sentIdx + 1 < sentences.length ? sentences[sentIdx + 1].start : words.length
	...
	const fontPickerModal = showFontPicker && (
		...
		>
			...
			>
				...
				<h2 className={`text-lg font-bold ${c.textStrong} mb-4`}>Reading Font</h2>
				<div className="flex flex-col gap-3">
					{(Object.entries(FONT_CONFIG) as [FontChoice, (typeof FONT_CONFIG)[FontChoice]][]).map(
						([key, cfg]) => (
							<button
								key={key}
				...
				<div className="max-w-lg w-full">
					<div className="mb-8 flex items-center justify-between">
						<div className="flex items-center gap-3">
							<h1 className={`text-2xl font-bold ${c.textStrong} font-display`}>RSVP Reader</h1>
							<button
					...
					<p className={`${c.textMuted} text-sm mb-6 -mt-4`}>
						...
						<span className={c.textFaint}>(All data stays on your device.)</span>
					</p>
					...
					{showInfo && (
						...
						>
							...
							>
								...
								<h2 className={`text-lg font-bold ${c.textStrong} mb-3`}>What is RSVP?</h2>
								<div className={`text-sm ${c.textMuted} leading-relaxed space-y-3`}>
									<p>
										<strong className={c.text}>Rapid Serial Visual Presentation</strong> shows you
										one word at a time, with a highlighted anchor letter so your eyes stay fixed in
										one spot. Words come to you instead of you scanning across a page.
									</p>
									<p>
										Most people can comfortably read at 300–500 wpm this way — much faster than
										normal reading — because your eyes never need to move.
									</p>
									<p>
										This is a free tool — use it as much as you like. Your text never leaves your
										device. Your reading position is bookmarked in your browser, so you can come
										back later and pick up where you left off. Just drop the same file in the same
										browser, and it will resume automatically.
									</p>
									<a
					...
					<div className="flex flex-col gap-3.5">
						<div
							className={`border rounded-xl px-6 py-5 cursor-pointer flex items-center gap-4 transition-colors ${
								dragOver ? 'border-amber-500 bg-amber-50/10' : `${c.inputBorder} ${c.cardBg}`
							}`}
						...
						>
							...
							<div>
								<p className="font-semibold text-sm">
									{loading ? 'Extracting text…' : 'Drop file or click to select'}
								</p>
								<p className={`${c.textMuted} text-xs mt-0.5`}>.pdf · .txt · .md</p>
							</div>
							<input
								ref={fileInputRef}
								type="file"
						...
						<button
							...
							onClick={() =>
								pasteText.trim() &&
								startReading(pasteText, pasteText.trim().split(/\s+/).slice(0, 5).join(' ') + '…')
							}
						>
							Start Reading →
						</button>
									...
									return (
										...
										>
											...
											{hasText ? (
												...
												<p className={`${c.textFaint} text-xs shrink-0`}>Re-upload to resume →</p>
											)}
										</div>
									)
								})}
		...
		>
			...
			>
				...
				<div className="flex-1 md:hidden h-2 relative">
					<div className={`${c.progressBg} rounded-md h-full relative overflow-hidden`}>
						<div
				...
				<div className="hidden md:flex flex-1 flex-col max-h-80">
					<div className={`flex-1 ${c.progressBg} rounded-md relative overflow-hidden`}>
						<div
			...
			<div className="flex flex-col items-center justify-center relative order-3 md:order-none min-h-[200px]">
				...
				<div className="relative w-full" style={{ height: '4.5rem' }}>
					...
					>
						...
						<span className="text-amber-500" style={{ textShadow: c.orpShadow }}>
							{orp}
						</span>
				...
				{/* Text name */}
				{textName && (
					<p className={`absolute bottom-4 text-[11px] ${c.textFaint} truncate max-w-xs`}>
						{textName}
					</p>
				)}
			</div>
						...
						return (
							<div key={s.start} className="relative">
								<button
									...
									onTouchStart={() => {
										longPressTimer.current = setTimeout(() => setHoveredSentence(i), 400)
									}}
									onTouchEnd={() => {
			...
			>
				...
				<div className="flex items-center gap-2.5 flex-1 min-w-[140px] max-w-72">
					<span className={`text-xs ${c.textMuted} whitespace-nowrap`}>{wpm} wpm</span>
					<input
						type="range"
						min={80}
						max={1000}
				...
				<span
					onClick={() => {
						const input = window.prompt(`Jump to word index (0–${words.length - 1}):`, String(idx))
						if (input !== null) {
